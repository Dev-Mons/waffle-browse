...
- **빠른 검색**
  ...
  - 현재 사용자 권한으로 접근 가능한 경로만 관리형 파일 열거 API로 색인
  - 검색 입력란에 포커스할 때 활성 탭의 현재 폴더만 필요 시점에 색인
  - 색인 완료 후 `FileSystemWatcher`로 해당 폴더의 실행 중 변경 반영
  - 접근 거부 경로를 건너뛰고 reparse point를 따라가지 않는 최소 권한 설계
  - 활성 탭의 현재 폴더와 하위 폴더만 검색
...
검색은 Waffle 자체 인덱스를 사용합니다. 앱 시작 시 미리 색인하지 않습니다. 검색 입력란을 선택하면 활성 탭의 현재 폴더와 하위 폴더만 색인하며, 다른 탭의 폴더에서 검색을 준비하면 기존 색인 범위를 새 폴더로 교체합니다. 색인 완료 후 실행 중 생성·삭제·이름 변경은 파일시스템 알림으로 반영합니다.
...
`build-exe.bat`는 framework-dependent WPF 앱을 디버그 심볼 없이 `publish\win-x64`에 게시합니다. 별도 서비스, helper, 드라이버 또는 관리자 권한은 사용하지 않습니다.
...
색인 범위는 사용자가 검색을 준비한 활성 탭 폴더 하나로 제한됩니다. 앱은 현재 사용자가 정상적으로 접근할 수 없는 항목을 건너뜁니다. 원시 볼륨, MFT/USN, 권한 상승, 보안 설정 변경은 사용하지 않습니다. 자세한 경계는 [안전한 파일 색인 설계](docs/waffle-native-file-index-plan.md)에 정리되어 있습니다.
